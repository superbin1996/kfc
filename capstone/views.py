...
from .models import Category, Picture, Dish, Selection, User, Voucher, Order, Deal
...
def item_images(request, item_id):
    ...
    Get items for each category
    """
    images = Dish.objects.get(id=item_id).images.values('image')
    # ic(images)
...
def get_deals(request):
    deals = Deal.objects.values(
        'id', 'title', 'dish__category', 'image', 'hashtag', 'start', 'end', 'description')
